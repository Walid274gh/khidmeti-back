...
export class HfQueueService {
  ...
  constructor(config: ConfigService) { this.base = config.get<string>('HF_SPACE_URL')?.replace(/\/+$/, '') || undefined; }
  get enabled(): boolean { return !!this.base; }
  async call(fnIndex: number, data: unknown[]): Promise<string> {
    if (!this.base) { if (!this.warnedNoBase) { this.warnedNoBase = true; this.logger.warn('HF_SPACE_URL not set'); } throw new Error('HF queue disabled'); }
    const session = `kh-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;
    const joinRes = await fetch(`${this.base}/queue/join`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ data, fn_index: fnIndex, session_hash: session }), signal: AbortSignal.timeout(15_000) });
    ...
    while (Date.now() < deadline) {
      const sseRes = await fetch(`${this.base}/queue/data?session_hash=${session}`, { headers: { Accept: 'text/event-stream' }, signal: AbortSignal.timeout(15_000) });
